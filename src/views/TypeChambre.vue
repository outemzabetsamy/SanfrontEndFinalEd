<template>
    <v-container fluid>
      ...
      <v-card>
        <v-card-title>
          <span class="title">
             Types Chambres ({{allTypeChambres.length}})
          </span>
          <v-spacer></v-spacer>
	...
	<tabledata :headers="headers" :items="allTypeChambres"  @edit="edit"
...
export default {
    ...
    data() {
        return {
            headers:[
                ...
                {text:"Designation",value:"nomType",sortable:true},
                {text:"Nb max personnes",value:"nbMaxPers",sortable:true},
                {text:"Tarif adulte",value:"prixAdulte",sortable:false},
                {text:"Tarif enfant",value:"prixEnfant",sortable:false},
    ...
    computed:mapGetters(['allTypeChambres','getUser','getToken']),
    methods:{
      ...mapActions(['fetchAllTypeChambres','deleteTypeChambre','whoAMI']),
      add(){
            console.log("ikechmed")
            this.$router.push({name:'TypeChambreForm'});
        },
        edit(item){
            console.log(item);
            this.$router.push(`/typechambres/${item.id}`)
        },
        remove(item){
            this.itemId=item.id;
            this.dialog=true;
